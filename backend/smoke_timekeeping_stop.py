...
_, stopped = request("PATCH", f"/api/v1/timekeeping/crew-logs/{log_id}/segments/stop", {}, token=token)

if stopped.get("id") != seg_id:
    raise SystemExit(f"FAIL: stop returned id={stopped.get('id')} expected id={seg_id}")

if stopped.get("end_at") is None:
    raise SystemExit("FAIL: end_at is null after stop")
...
print("end_at:", stopped.get("end_at"))
